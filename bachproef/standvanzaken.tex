...
\begin{itemize}
    \item \textbf{LLM-modellen voor Retrieval-Augmented Generation (RAG)} – een analyse van beschikbare modellen en hun geschiktheid voor RAG.
...
Welke bestaande LLM-modellen kunnen worden gebruikt voor het ontwikkelen van een Retrieval-Augmented Generation (RAG)?
        ...
        Hoewel het doel van beide benchmarks hetzelfde is is de manier van werken wel anders. ChatbotArena gaat via gebruikers twee anonieme LLM modellen tegenover elkaar plaatsen en de gebruiker laten nagaan welke van de 2 het beste resultaat heeft opgeleverd.
        
        Het voordeel van deze methode is dat de LLM-modellen realistische cases moeten behandelen die door gebruikers zelf worden gesteld. Op basis van resultaten die de LLM modellen tonen kan een gebruiker zijn voorkeur meegeven. Op basis van deze stemmen worden verscheidene modellen tegenover elkaar geplaatst. In januari 2024 werden ruim 240.000 stemmen uitgebracht door ongeveer 90.000 gebruikers \autocite{chiang2024chatbot}. 
